  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between space-y-4 md:flex-row md:items-center md:space-y-0 pb-4">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white">{t("Farm Registry Center")}</h1>
        ...
        <Button onClick={handleOpenCreate} className="bg-primary hover:bg-primary-600 text-white shadow-premium rounded-xl px-6 py-5">
          <Plus className="mr-2 h-5 w-5" /> {t("Register Land Parcel")}
        </Button>
      </div>

      {/* ─── SUMMARY STATS ROW ─────────────────────────────────────────────────── */}
      <div className="grid gap-6 sm:grid-cols-4">
        <Card className="hover:border-slate-300 dark:border-white/20 transition-all rounded-[2rem] shadow-sm">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">{t("Total Registered Farms")}</span>
              <p className="text-4xl font-black text-slate-900 dark:text-white mt-2">{farms.length}</p>
            </div>
            <div className="h-12 w-12 rounded-2xl bg-primary/10 text-primary dark:text-primary-300 border border-primary/20 flex items-center justify-center">
        ...
        <Card className="hover:border-slate-300 dark:border-white/20 transition-all rounded-[2rem] shadow-sm">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">{t("Aggregate Acreage")}</span>
              <p className="text-4xl font-black text-slate-900 dark:text-white mt-2">{totalAcreage.toFixed(1)} <span className="text-lg font-semibold text-slate-400">{t("Ha")}</span></p>
            </div>
            <div className="h-12 w-12 rounded-2xl bg-primary/10 text-primary dark:text-primary-300 border border-primary/20 flex items-center justify-center">
        ...
        <Card className="hover:border-slate-300 dark:border-white/20 transition-all rounded-[2rem] shadow-sm">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">{t("Mean Crop Health")}</span>
              <p className="text-4xl font-black text-slate-900 dark:text-white mt-2">{avgHealth}%</p>
            </div>
            <div className="h-12 w-12 rounded-2xl bg-primary/10 text-primary dark:text-primary-300 border border-primary/20 flex items-center justify-center">
        ...
        <Card className="hover:border-slate-300 dark:border-white/20 transition-all rounded-[2rem] shadow-sm">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">{t("Device Alert Status")}</span>
              <p className="text-4xl font-black text-slate-900 dark:text-white mt-2">{activeAlerts}</p>
            </div>
            <div className="h-12 w-12 rounded-2xl bg-status-critical/10 text-status-critical border border-status-critical/20 flex items-center justify-center">
      ...
      <div className="space-y-6">
        {farms.map((farm) => {
          ...
          return (
            <Card key={farm.id} className="relative overflow-hidden transition-all hover:border-slate-300 dark:hover:border-white/20 shadow-premium rounded-[2rem] border-slate-200/70 dark:border-white/10">
              {/* Health color bar indicator */}
              <div className={`absolute top-0 left-0 right-0 h-2 ${
                farm.health_score >= 80 ? 'bg-primary' : 
                farm.health_score >= 60 ? 'bg-status-warning' : 'bg-status-critical'
              }`} />

              <CardContent className="p-6 pt-8 space-y-6">
                
                {/* Section 1: Header name and primary parameters */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-white/5/60 pb-5">
                  <div className="space-y-1">
                    <div className="flex items-center space-x-2">
                      <h3 className="text-2xl font-black text-slate-900 dark:text-white">{farm.name}</h3>
                      <Badge variant={farm.health_score >= 80 ? 'success' : farm.health_score >= 60 ? 'warning' : 'destructive'} className="py-0.5 px-2.5 text-xs font-bold rounded-lg">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400 font-semibold">
                      ...
                      <span className="flex items-center"><Sprout className="h-3.5 w-3.5 mr-1 text-primary dark:text-primary-300" /> {t("Crop")}: {farm.crop_type}</span>
                    </div>
                  </div>

                  <div className="flex items-center space-x-2 shrink-0">
                    <Button onClick={() => handleOpenEdit(farm.id)} variant="outline" size="sm" className="h-8 text-xs">
                      <Edit className="mr-1.5 h-3.5 w-3.5 text-slate-600 dark:text-slate-400" /> {t("Edit Info")}
                    </Button>
                ...
                <div className="grid gap-6 md:grid-cols-5">
                  ...
                  <div className="md:col-span-3 grid gap-4 sm:grid-cols-2 text-xs">
                    ...
                    <div className="bg-background dark:bg-dark-bg/40 border border-slate-200 dark:border-white/10/20 p-3.5 rounded-xl space-y-1">
                      ...
                      <p className="font-bold text-slate-800 dark:text-slate-200 flex items-center">
                        <Activity className="h-3.5 w-3.5 mr-1.5 text-primary dark:text-primary-300" /> {meta.growthStage} {t("Stage")}
                      </p>
                    </div>
                    ...
                    <div className="bg-background dark:bg-dark-bg/40 border border-slate-200 dark:border-white/10/20 p-3.5 rounded-xl space-y-1">
                      ...
                      <p className="font-bold text-slate-800 dark:text-slate-200 flex items-center">
                        <CloudSun className="h-3.5 w-3.5 mr-1.5 text-primary dark:text-primary-300" /> {meta.weatherZone}
                      </p>
                    </div>
                    ...
                    <div className="bg-background dark:bg-dark-bg/40 border border-slate-200 dark:border-white/10/20 p-3.5 rounded-xl space-y-1">
                      ...
                      <p className="font-bold text-slate-800 dark:text-slate-200 flex items-center">
                        <Layers className="h-3.5 w-3.5 mr-1.5 text-primary dark:text-primary-300" /> {meta.satelliteCoverage}
                      </p>
                    </div>
                    ...
                    <div className="bg-background dark:bg-dark-bg/40 border border-slate-200 dark:border-white/10/20 p-3.5 rounded-xl space-y-1">
                      ...
                      <p className="font-bold text-slate-800 dark:text-slate-200 flex items-center">
                        <LayoutGrid className="h-3.5 w-3.5 mr-1.5 text-slate-500 dark:text-slate-400" /> {farm.area_ha} {t("Hectares")}
                      </p>
                    </div>
                  ...
                  {/* SVG Map (2 cols) */}
                  <div className="md:col-span-2 h-40 border border-slate-200 dark:border-white/10/70 rounded-xl bg-slate-900 relative overflow-hidden flex items-center justify-center">
                    {/* Dark Grid Backdrop */}
                    ...
                    <div className="absolute bottom-2 left-2 bg-slate-950/80 px-2 py-0.5 rounded text-[8px] border border-slate-800 font-bold text-slate-400 flex items-center">
                      <MapIcon className="h-3 w-3 mr-1 text-primary dark:text-primary-300" /> IoT Sensor Pins Active
                    </div>
                  </div>
                ...
                <div className="border-t border-slate-100 dark:border-white/5/70 pt-4 space-y-2">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Active Edge Mesh Devices</span>
                  <div className="flex flex-wrap gap-2">
                    <Badge variant="outline" className="flex items-center space-x-1.5 bg-background dark:bg-dark-bg border-slate-200 dark:border-white/10/60">
                      <Cpu className="h-3 w-3 text-primary dark:text-primary-300" />
                      <span className="font-semibold text-slate-700 dark:text-slate-300">IoT_NODE_01 (Soil/Temp)</span>
                    </Badge>
                    <Badge variant="outline" className="flex items-center space-x-1.5 bg-background dark:bg-dark-bg border-slate-200 dark:border-white/10/60">
                      <Cpu className="h-3 w-3 text-primary dark:text-primary-300" />
                      <span className="font-semibold text-slate-700 dark:text-slate-300">IoT_NODE_02 (Soil/NPK)</span>
                    </Badge>
                    <Badge variant="outline" className="flex items-center space-x-1.5 bg-background dark:bg-dark-bg border-slate-200 dark:border-white/10/60">
                      <Cpu className="h-3 w-3 text-primary dark:text-primary-300" />
                      <span className="font-semibold text-slate-700 dark:text-slate-300">IoT_NODE_03 (Soil pH)</span>
                    </Badge>
                    <Badge variant="outline" className="flex items-center space-x-1.5 bg-background dark:bg-dark-bg border-slate-200 dark:border-white/10/60">
                      <Shield className="h-3 w-3 text-highlight dark:text-highlight-300" />
                      <span className="font-semibold text-slate-700 dark:text-slate-300">LORA_GATEWAY_HUB_01</span>
                    </Badge>
                  </div>
                </div>
        ...
        <form onSubmit={handleCreateOrUpdate} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center px-0.5">
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Field Boundary</label>
              <span className="text-[9px] text-primary dark:text-primary-300 bg-primary-50 dark:bg-primary-900/30 px-1.5 py-0.5 rounded-full font-bold">Draw to auto-calculate area</span>
            </div>
            <div className="border border-slate-200 dark:border-white/10 rounded-xl overflow-hidden shadow-inner">
              <FarmMap 
                mode="edit" 
                polygon={polygon} 
                onChange={(geo, areaHa) => {
          ...
          {/* Footer buttons */}
          <div className="flex space-x-3 pt-3 border-t border-slate-100 dark:border-white/5/50 justify-end mt-4">
            <Button
              type="button"
              variant="outline"
              disabled={loading}
